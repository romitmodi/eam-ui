  ...
  const SelProjectCall = (e: any) => {
    ...
    projects.forEach((data) => {
      if (data.name === e.target.value)
        data.labDetails.forEach((lab) => {
          labDetails.push(<option value={lab.name}>{lab.name}</option>);
        });
    });
    ...
    projects.forEach((proj) => {
      proj.labDetails.forEach((lab) => {
        if (lab.name === e.target.value) {
          lab.teamDetails.forEach((team) => {
            teamDetails.push(<option value={team.id}>{team.name}</option>);
          });
        }
      });
    });
      ...
      <form className="form">
        ...
        <div>
          ...
          <select
            ...
            id="BU"
            defaultValue={["Select"]}
            onChange={(event) => SelProjectCall(event)}
          >
        ...
        <br />
        <button
          className="lyds-button"
          onClick={() => alert("Submit successfully")}
        >
          Submit Request
        </button>
      </form>
    ...
    <>
      <div>
        ...
        <select name="FT" id="FT">
          {projects[0].labDetails[0].teamDetails.map((team) => (
